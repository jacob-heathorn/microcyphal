...
constexpr std::uint32_t omit_serialization_support = 0;
...
constexpr std::uint32_t std_flavor = 2826573480;
...
constexpr std::uint32_t variant_include = 0;
...
public:
    ...
    derived_bitspan subspan(
        const std::size_t bits_at = 0,
        const std::size_t size_bits = std::numeric_limits<std::size_t>::max()
        ) const noexcept {
        auto& self = *static_cast<const derived_bitspan*>(this);
        const std::size_t offset_bits = self.offset_bits_ + bits_at;
        const std::size_t offset_bytes = offset_bits / 8U;
        const std::size_t offset_bits_mod = offset_bits % 8U;
        std::size_t new_size_bytes = 0U;
