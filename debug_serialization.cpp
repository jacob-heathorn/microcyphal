...
int main() {
    std::cout << "=== Testing C++ Heartbeat Serialization Bug ===" << std::endl;
    ...
    std::cout << "\nExpected buffer size: " << kBufSizeBytes << " bytes" << std::endl;

    bitspan out_span(raw_buf, kBufSizeBytes);
    SerializeResult ser_ret = serialize(msg_orig, out_span);
    
    std::cout << "Serialize result: " << ser_ret.value() << " bytes" << std::endl;
